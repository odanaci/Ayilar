...
%2nd
 Epr=exp(-(r/(0.5*w_0)).^2);%.*besselj(0,beta0*r);
 %1st
  Epr=exp(-(r/(1.5*w_0)).^2);%.*besselj(0,beta0*r);
...
E=propTF(E,L_x,0.6,0.02); %simple model
...
% E=rect(Y*5).*exp(-(r/(0.2*w_0)).^2);
% E=propTF(E,L_x,0.6,0.0978);

%1st
% E=rect(Y*5).*exp(-(r/(0.7*w_0)).^2);
% E=propTF(E,L_x,0.6,0.5);
% E=propTF(E,L_x,0.6,0.2);



Ipu=abs(E).^2;
% Ipu=abs(exp(-(r/(w_0)).^2).*sinc(Y*3));
surfMe(Ipu.*abs(Epr).^2)
pause;

BB=zoom_(TCfilter(abs(fourt2(Ipu)),X,Y,2,0.2),0.15,0.85);
AA=zoom_(fliplr(TCfilter(abs(fourt2(Ipu)),X,Y,2,0.2)),0.15,0.85);

A=[AA,BB];
A=nthroot(A,2);
surfMe(A);set(gca,'xtick',[]);set(gca,'ytick',[]);
print(gcf,'SimpleRoot','-dpng');
...
theta=20*pi/180;
% deltaKz=DeltaKz(X,Y,theta,2,Ipu,kPu,kPr,kCo,npu,npr);
% deltaKz=DeltaKz(X,Y,theta,2,normalize_this(Ipu),kPu,kPr,kCo,npu,npr);

% surfMe(deltaKz);colorbar;
% surfMe(real(Epr.*exp(1i*deltaKz*h)));
% surfMe(imag(Epr.*exp(1i*deltaKz*h)));
...
% pause;
% deltaKz_=DeltaKzA(X,Y,Ipu,3,0.14,1,0.05);

% deltaKz_=DeltaKzA(X,Y,Ipu,0,0.9,1,0.05);%2nd
deltaKz_=DeltaKzA(X,Y,Ipu,0,1.7,1,0.02); %1st


surfMe(abs(exp(-deltaKz_*h).*Epr).^2)
pause;
clear Ipu;

amax=max(abs(Epr(:).^2));
% f=@(z,A)5*[i*kPr*Xi_pc*exp(1i*deltaKz*z).*conj(A(end/2+1:end,:))/2;...
%     i*kCo*Xi_cp*exp(1i*deltaKz*z).*conj(A(1:end/2,:))/2];
f=@(z,A)gain*[i*kPr*Xi_pc*exp(1i*deltaKz_*z).*conj(A(end/2+1:end,:))/2;...
    i*kCo*Xi_cp*exp(1i*deltaKz_*z).*conj(A(1:end/2,:))/2];
% f=@(z,A)5i*[kPr*Xi_pc*exp(-1*deltaKz_*z).*conj(A(end/2+1:end,:))/2;...
...
Epr=fftBpmDisp(Epr,L_x,2*pi/(kPr),Xi_pp,(h/2)/dif);
Econj=fftBpmDisp(Epr,L_x,2*pi/(kCo),Xi_cc,(h/2)/dif);
...
% deltaKz=DeltaKz(X,Y,theta,2,normalize_this(abs(E).^2),kPu,kPr,kCo,npu,npr);
% deltaKz_=deltaKz;deltaKz_(deltaKz>1)=1e5;%deltaKz_(deltaKz<=1)=0;
